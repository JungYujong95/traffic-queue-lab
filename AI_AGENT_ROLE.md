...
## 12. Work Planning Rule

사용자가 기능 개발, 리팩토링, 인프라 구성, 테스트 작성, 문서화 등 작업을 요청하면 구현 전에 반드시 `/docs/plan` 하위에 작업 계획 문서를 작성한다.

### Plan Document Rules

- 계획 문서는 작업 목적, 범위, 단계, 검증 방법을 포함한다.
- 계획 문서는 실제 구현 방향과 일치해야 한다.
- 작업 중 설계나 구현 방향이 바뀌면 계획 문서도 함께 갱신한다.
- 작업자는 작성한 계획 문서를 기준으로 구현을 진행한다.
- 계획 없이 바로 구현하지 않는다.
- 단순 질의응답, 명령어 안내, 상태 확인처럼 코드 변경이 없는 짧은 요청은 계획 문서 작성 대상에서 제외할 수 있다.
...
계획 문서의 이름은 작업 내용을 이해할 수 있도록 명확하게 작성한다.

---

## 13. Session Bootstrap Rule
